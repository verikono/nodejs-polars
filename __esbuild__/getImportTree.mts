...
export async function getImportTree(
  root: string | string[] | URL | URL[],
  options: getImportTreeParams = getImportTreeDefaults,
): Promise<URL[]> {
  ...
  const rootNames = (Array.isArray(root) ? root : [root])
    ...
    .map((filePath) =>
      isAbsolute(filePath) ? filePath : pathJoin(cwd.pathname, filePath),
    );
...
export async function resolveFileFromSpecifier(
  spec: string,
  base: URL,
): Promise<URL> {
...
if (import.meta.vitest) {
  ...
  describe("getImportTree", () => {
    it("getImportTree is a function", async () =>
      expect(getImportTree).toBeTypeOf("function"));
    it("parses typescript imports when argued a URL", async () => {
      const result = await getImportTree(
        new URL("../polars/index.ts", import.meta.url),
      );
      console.log(result);
    });
