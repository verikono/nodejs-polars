...
import { type URLSpec } from "./utils/xurl/URLSpec.mjs";
 ...
 * @returns
 */
export function resolveTSEntryToSpec(
  entry: string | URL,
  base?: string | URL,
): string {
  ...
  const candidates = [""]
    .concat(Object.values(translateExtMap), Object.keys(translateExtMap))
    .map((v) => (v ? `index${v}` : v));
...
if (import.meta.vitest) {
  ...
  describe("resolveTSEntryToSpec", () => {
    ...
    it(`correctly parses with arguments ( '../__esbuild__/index.mts', import.meta.url )`, () => {
      const result = resolveTSEntryToSpec(
        "../__esbuild__/index.mts",
        import.meta.url,
      );
      expect(result).toBeTypeOf("string");
